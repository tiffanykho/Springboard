...
FROM Facilities
WHERE membercost < (monthlymaintenance * .20) AND membercost > 0.0;
...
SET @a=1;
PREPARE STMT FROM 'SELECT firstname, surname FROM Members ORDER BY joindate DESC LIMIT ?';
...
ON b.facid = f.facid
WHERE b.memid != 0 AND (f.facid = 0 OR f.facid = 1)
ORDER BY full_name
...
SELECT DISTINCT CONCAT(m.firstname, ' ', m.surname) AS full_name, f.name AS facility_name, 
CASE WHEN b.memid = 0 
...
SELECT MONTH(starttime) AS month, f.name AS facility_name, COUNT(b.memid) AS member_usage 
FROM Bookings AS b 
LEFT JOIN Facilities AS f
ON b.facid = f.facid
WHERE memid != 0 
